...
def configure_oracle_ssl_on_sauropod(config: ConfigLoader, logger, verbose: bool = True) -> bool:
    if verbose:
        logger.info("=" * 80)
        logger.info("Configuring SSL/TLS listener for Oracle on sauropod")
        logger.info("=" * 80)
    ...
    try:
        with SSHClient(
            ...
            if verbose:
                logger.info("Step 1: Updating listener configuration for SSL")
            ...
            if verbose:
                logger.info("=" * 80)
                logger.info("Oracle SSL/TLS listener configuration completed successfully!")
                logger.info("Network Configuration:")
                logger.info("  - listener.ora: TCP on port 1521, TCPS on port 2484")
                logger.info("  - tnsnames.ora: ORCLPDB1 (TCP), ORCLPDB1_SSL (TCPS)")
                logger.info("  - sqlnet.ora: SSL/TLS enabled with wallet")
        ...
        logger.error(f"Error during Oracle SSL configuration: {e}")
        return False
